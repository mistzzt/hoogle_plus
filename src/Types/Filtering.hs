...
data CandidateValidDesc =
    Total   [InternalExample] [[InternalExample]] [InternalExample]
  | Partial [InternalExample] [[InternalExample]] [InternalExample]
  | Invalid
  | Unknown String
  deriving (Eq)
...
instance Show CandidateValidDesc where
  show = \case
      Total   examples _ _ -> unlines $ map show examples
      Partial examples _ _ -> unlines $ map show examples
      Invalid              -> "<bottom>"
...
emptyFilterState = FilterState {
  ...
  higherOrderArgumentCache = Map.fromList [
    ("((Int) -> (Int))", ["const 5", "\\x -> x * x", "id", "\\x -> x + 1"]),
    ("[Int] -> Int", ["head", "last", "length", "\\xs -> xs !! 1"]),
    ("((Int) -> (Bool))", ["\\x -> x < 0", "\\x -> x > 0"]),
    ("((Int) -> (String))", ["show", "\\x -> \"num=\" ++ show x ++ \";\""]),
    ("((Int) -> (((((Either) (String))) (Bool))))", ["\\x -> if x == 0 then Left \"Zero\" else Right (x < 0)"])
  ]
}
